...
import { ListVector2Picker } from '@/components/molecules/ListVector2Picker'
import {
  ParameterPickerModal,
  type ParameterPickerAnchor,
} from '@/components/molecules/ParameterPickerModal'
import {
  formatListVector2Preview,
...
type ParameterListVector2InputProps = {
  ariaLabel: string
  className?: string
  value: string
...
export function ParameterListVector2Input({
  ariaLabel,
  className,
  value,
  onCommit,
  onFocusChange,
}: ParameterListVector2InputProps) {
  const [open, setOpen] = useState(false)
  const [popoverUp, setPopoverUp] = useState(false)
  const wrapRef = useRef<HTMLDivElement>(null)
  const [anchor, setAnchor] = useState<ParameterPickerAnchor | null>(null)

  const items = parseListVector2String(value)
  const previewLabel = formatListVector2Preview(items)
  ...
  return (
    <div className={[styles.wrap, className ?? ''].filter(Boolean).join(' ')} ref={wrapRef}>
      <button
        aria-label={`${ariaLabel} — abrir editor List[Vec2]`}
        className={styles.preview}
        onClick={() => (open ? closePicker() : openPicker())}
        style={{ color: 'var(--syntax-vector2)' }}
        type="button"
      >
        {items.length}
      </button>
      <input
        ...
        data-parameter-type="listVector2"
        onClick={() => (open ? closePicker() : openPicker())}
        onFocus={() => openPicker()}
      ...
      {anchor ? (
        <ParameterPickerModal
          anchor={anchor}
          ariaLabel={`${ariaLabel} — List[Vec2]`}
          layerTestId="listVector2"
          onClose={closePicker}
          open={open}
        ...
        >
          <ListVector2Picker
            onChange={(next) => onCommit(normalizeListVector2String(next))}
            value={value}
          />
        </ParameterPickerModal>
      ) : null}
