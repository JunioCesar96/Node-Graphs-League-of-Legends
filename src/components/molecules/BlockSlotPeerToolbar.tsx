...
type BlockSlotPeerToolbarProps = {
...
export function BlockSlotPeerToolbar({
  peer,
  onToggleLock,
  onToggleVisibility,
  onFocusPeer,
  onRemoveConnection,
}: BlockSlotPeerToolbarProps) {
  ...
  return (
    <div className={styles.toolbar} onPointerDown={(event) => event.stopPropagation()}>
      ...
      <button
        aria-label="Focar slot do nó ligado"
        className={styles.action}
        onClick={(event) => {
          event.stopPropagation()
          onFocusPeer()
        }}
        title="Focar slot do nó ligado"
        type="button"
      >
        <SceneNodeFocusIcon />
      </button>
      ...
      <button
        aria-label="Remover ligação do slot"
