...
export type EditParameterDialogProps = {
  isOpen: boolean
  onCancel: () => void
  onConfirm: (input: ManualBlockParameterFormInput) => void
}

export function EditParameterDialog({ isOpen, onCancel, onConfirm }: EditParameterDialogProps) {
  const { t } = useLanguage()
  const titleId = useId()
  ...
  if (formOpen && selectedParameter) {
    return (
      <CreateParameterDialog
        editDocument={selectedParameter}
        fixedBlockName={selectedParameter.block.trim()}
        isOpen
        nested
  ...
  const canContinue = Boolean(selectedParameter) && !loading
    ...
    >
      <div className={styles.panel}>
        <p className={styles.title} id={titleId}>
          {t(LangId.EditParameterDialogTitle, 'Editar parâmetro do catálogo')}
        </p>
        <p className={styles.hint}>
          {t(LangId.EditParameterDialogHint, 'Seleccione o parâmetro e edite os campos.')}
        </p>
        ...
        {blockName ? (
          <div className={styles.field}>
            <span>{t(LangId.EditParameterDialogSelectParameter, 'Parâmetro')}</span>
            <EmbeddedStructureListPicker
              emptyHint={t(LangId.EditParameterDialogNoParameters, 'Nenhum parâmetro neste bloco.')}
              items={parameterListItems}
              listTitle={t(LangId.EditParameterDialogSelectParameter, 'Parâmetro')}
        ...
        <div className={styles.actions}>
          ...
          <button
            ...
            type="button"
          >
            {t(LangId.EditParameterDialogContinue, 'Editar')}
          </button>
        </div>
      </div>
    </div>,
