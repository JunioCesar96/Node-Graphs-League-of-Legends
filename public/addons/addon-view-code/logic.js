...
const FULLSCREEN_ROOT_PREFIX = 'addon-view-code-fullscreen-'
const FULLSCREEN_STYLE_ID = 'addon-view-code-fullscreen-styles'
...
function rootElement(cardDOM) {
  const el = cardDOM.querySelector('.addon-view-code-ui')
...
function ensureFullscreenStyles() {
  ...
  style.textContent = `
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-backdrop {
      position: absolute;
      inset: 0;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-panel {
      position: relative;
      z-index: 1;
      display: flex;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-header {
      display: flex;
      align-items: center;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-title {
      font-size: 14px;
      font-weight: 600;
      color: #e2e8f0;
    }

    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-close {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      width: 32px;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-close:hover {
      color: #f8fafc;
      border-color: #22c55e;
    }

    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-view-code-fullscreen-textarea {
      flex: 1 1 auto;
      min-height: 0;
      width: 100%;
      box-sizing: border-box;
...
function ensureFullscreenOverlay(cardDOM) {
  ...
  overlay.hidden = true
  overlay.innerHTML = `
    <div class="addon-view-code-fullscreen-backdrop" data-action="collapse"></div>
    <div class="addon-view-code-fullscreen-panel" role="dialog" aria-modal="true">
      <div class="addon-view-code-fullscreen-header">
        <span class="addon-view-code-fullscreen-title">View Code</span>
        <button type="button" class="addon-view-code-fullscreen-close" data-action="collapse" aria-label="${collapseLabel}" title="${collapseLabel}">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
            <path d="M18 6L6 18M6 6l12 12" />
          </svg>
        </button>
      </div>
      <textarea class="addon-view-code-fullscreen-textarea" readonly spellcheck="false"></textarea>
    </div>
  `
...
function setExpanded(cardDOM, expanded) {
  ...
  const fullscreenTextarea = overlay.querySelector('.addon-view-code-fullscreen-textarea')
