...
const FULLSCREEN_ROOT_PREFIX = 'addon-code-to-block-fullscreen-'
const FULLSCREEN_STYLE_ID = 'addon-code-to-block-fullscreen-styles'
...
function rootElement(cardDOM) {
  const el = cardDOM.querySelector('.addon-code-to-block-ui')
 ...
 */
function codeTextarea(cardDOM) {
  const el = cardDOM.querySelector('[name="code"]')
...
function syncExpandButtonState(cardDOM) {
  ...
  const button = cardDOM.querySelector('#expand-code-btn')
...
function ensureFullscreenStyles() {
  ...
  style.textContent = `
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-backdrop {
      position: absolute;
      inset: 0;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-panel {
      position: relative;
      z-index: 1;
      display: flex;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-header {
      display: flex;
      align-items: center;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-title {
      font-size: 14px;
      font-weight: 600;
      color: #e2e8f0;
    }

    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-close {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      width: 32px;
    ...
    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-close:hover {
      color: #f8fafc;
      border-color: #a855f7;
    }

    [id^="${FULLSCREEN_ROOT_PREFIX}"] .addon-code-to-block-fullscreen-textarea {
      flex: 1 1 auto;
      min-height: 0;
      width: 100%;
      box-sizing: border-box;
...
function ensureFullscreenOverlay(cardDOM) {
  ...
  overlay.hidden = true
  overlay.innerHTML = `
    <div class="addon-code-to-block-fullscreen-backdrop" data-action="collapse"></div>
    <div class="addon-code-to-block-fullscreen-panel" role="dialog" aria-modal="true">
      <div class="addon-code-to-block-fullscreen-header">
        <span class="addon-code-to-block-fullscreen-title">Code to Block</span>
        <button type="button" class="addon-code-to-block-fullscreen-close" data-action="collapse" aria-label="${collapseLabel}" title="${collapseLabel}">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
            <path d="M18 6L6 18M6 6l12 12" />
          </svg>
        </button>
      </div>
      <textarea class="addon-code-to-block-fullscreen-textarea" spellcheck="false"></textarea>
    </div>
  `
...
function setExpanded(cardDOM, expanded) {
  const root = rootElement(cardDOM)
  const textarea = codeTextarea(cardDOM)
  if (!root || !textarea) {
    return
  }
  ...
  const fullscreenTextarea = overlay.querySelector('.addon-code-to-block-fullscreen-textarea')
...
function execute(inputs, cardDOM) {
  ...
  if (root?.getAttribute('data-expanded') === 'true') {
    ...
    const fullscreenTextarea = overlay?.querySelector('.addon-code-to-block-fullscreen-textarea')
    if (fullscreenTextarea instanceof HTMLTextAreaElement && fullscreenTextarea.value !== code) {
      fullscreenTextarea.value = code
    }
  }
